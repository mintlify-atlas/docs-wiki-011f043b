...
public interface CommandHandler<REQ, RES> {
    ...
    default RES fallback(Command<REQ> command, Throwable t) throws Throwable {
        throw t;  // override for custom fallback behaviour
    }
...
public interface CommandHookBefore<T>  { void onBefore(Command<T> command); }
public interface CommandHookAfter<T>   { void onAfter(Command<T> command, Object response); }
public interface CommandHookError<T>   { void onError(Command<T> command, Throwable t); }
```
...
`JdbcCommandStore` is active when no other `CommandStore` bean exists and `fineract.command.jdbc.enabled=true` (default). It maps between `Command<T>` and the Spring Data JDBC entity `CommandEntity`:
...
| Property | Env Var | Default | Description |
|---|---|---|---|
| `fineract.command.jdbc.enabled` | `FINERACT_COMMAND_JDBC_ENABLED` | `true` | Enable `JdbcCommandStore` |
