...
`updateRunningBalance()` is called by a scheduled job (`Update Accounting Running Balances`) that computes the cumulative balance for each GL account across all offices. The running balance is stored on `acc_gl_journal_entry.running_balance` and is used by financial reports that need a point-in-time balance without aggregating all historical rows.
