...
The concrete `LoanAccountLock` maps to table `m_loan_account_locks`. `LockOwner` is an enum with values `LOAN_COB_CHUNK_PROCESSING` and `LOAN_INLINE_COB_PROCESSING`.
