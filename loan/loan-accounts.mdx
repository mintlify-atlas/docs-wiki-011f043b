...
```java
// fineract-core/src/main/java/org/apache/fineract/portfolio/loanaccount/domain/LoanStatus.java (enum)
INVALID(0, ...),
...
|---|---|---|
| Submit application | `LoanApplicationSubmittalCommandHandler` | `LOAN_CREATED` |
...
| Undo disbursal | `UndoDisbursalLoanCommandHandler` | `LOAN_DISBURSAL_UNDO` |
...
All handlers are annotated with `@CommandType(entity = ..., action = ...)` and routed by `PortfolioCommandSourceWritePlatformService` via the `NewCommandSourceHandler` SPI.
...
| Class | Strategy code |
|---|---|
| `FineractStyleLoanRepaymentScheduleTransactionProcessor` | `mifos-standard-strategy` |
| `HeavensFamilyLoanRepaymentScheduleTransactionProcessor` | `heavensfamily-strategy` |
| `CreocoreLoanRepaymentScheduleTransactionProcessor` | `creocore-strategy` |
| `RBILoanRepaymentScheduleTransactionProcessor` | `rbi-india-strategy` |
| `PrincipalInterestPenaltyFeesOrderLoanRepaymentScheduleTransactionProcessor` | `principal-interest-penalties-fees-order-strategy` |
| `InterestPrincipalPenaltyFeesOrderLoanRepaymentScheduleTransactionProcessor` | `interest-principal-penalties-fees-order-strategy` |
| `EarlyPaymentLoanRepaymentScheduleTransactionProcessor` | `early-repayment-strategy` |
| `DuePenFeeIntPriInAdvancePriPenFeeIntLoanRepaymentScheduleTransactionProcessor` | `due-penalty-fee-interest-principal-in-advance-principal-penalty-fee-interest-strategy` |
| `DuePenIntPriFeeInAdvancePenIntPriFeeLoanRepaymentScheduleTransactionProcessor` | `due-penalty-interest-principal-fee-in-advance-penalty-interest-principal-fee-strategy` |

For progressive (actuarial) loans, `AdvancedPaymentScheduleTransactionProcessor` is used with strategy code `advanced-payment-allocation-strategy` — see [Progressive Loans](/loan/progressive-loan).
...
public interface LoanScheduleGenerator {
    ...
    Money getPeriodInterestTillDate(
        @NotNull LoanRepaymentScheduleInstallment installment,
