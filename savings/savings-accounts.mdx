...
| `nominal_annual_interest_rate_overdraft` | `nominalAnnualInterestRateOverdraft` | Separate overdraft interest rate |
...
| `lockin_period_frequency` / `lockin_period_frequency_enum` | `lockinPeriodFrequency` / `lockinPeriodFrequencyType` | Lock-in period for withdrawals |
| `deposit_type_enum` | `depositType` | Read-only discriminator mirror |
| `min_balance_for_interest_calculation` | `minBalanceForInterestCalculation` | Minimum balance threshold for interest eligibility |
...
The full set of states is defined in `fineract-core/.../portfolio/savings/domain/SavingsAccountStatusType.java`:
...
Defined in `fineract-core/.../portfolio/savings/SavingsAccountTransactionType.java`. Each variant carries an optional `TransactionEntryType` that drives the double-entry direction:
...
| `INITIATE_TRANSFER` | 12 | — | Account transfer initiation marker |
| `APPROVE_TRANSFER` | 13 | — | Account transfer approval marker |
| `WITHDRAW_TRANSFER` | 14 | — | Account transfer withdrawal marker |
| `REJECT_TRANSFER` | 15 | — | Account transfer rejection marker |
| `WRITTEN_OFF` | 16 | — | Write-off marker |
...
<Tip>
To trace which slab applies to a given account, inspect the `chartSlabs` collection on `InterestRateChart` and compare each slab's `from_period`/`to_period` and `amount_range_from`/`amount_range_to` against the account's tenure and balance. The `InterestRateChartSlabComparator` is used internally to order slabs before validation.
</Tip>
